...
记 latexmath:[dp[i\]]（提前暴露）表示子串 latexmath:[s[0,i\]] 被分隔为一些回文子串时，得到的最少子串数。
我们的目的是从 latexmath:[dp[0\]] 到 latexmath:[dp[i-1\]] 出发，得到 latexmath:[dp[i\]]。首先 latexmath:[dp[0\]] 肯定是 1。

对于字符串 latexmath:[s[0,i\]]，会被分隔成几个回文子串，比如分隔成这样 latexmath:[s[0,t_1\], s[t_1,t_2), ..., s[t_k,i\]]。
...
latexmath:[\{ s[t_1,i\], s[t_2,1\], ..., s[t_k,i\] \}]。由以上的分析可以知道，
latexmath:[dp[i\] = \min \limits_{1 \le a \le k} \{ dp[t_{a-1}\] + 1 \}]。另外，我们注意到，当
