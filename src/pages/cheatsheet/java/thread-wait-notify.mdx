...
每个 Java 对象上都有一个[监视器](/cheatsheet/java/thread-sync)，每个监视器都有一个与之关联的**等待集合**（_wait set_）。一个等待集合是一个线程的集合。
...
这组方法配合[监视器锁](/cheatsheet/java/thread-sync)，可以进行线程执行顺序的编排。

- `wait` 方法执行后，当前线程进入 `WAITING` 状态，等待其他线程唤醒。
...
- [监视器锁](/cheatsheet/java/thread-sync)
